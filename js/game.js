...
// flame image
	...
	var reset = function () 
	{
        ...
        iframe.src = "https://www.youtube.com/embed/pLkC_uxWmDI";
	...
	{	
		...
		else if(backgroundCode == 6)
		{
            ...
            {
                if (scorePoints > highScores[0]) // High Score
                {
                    previousScore = highScores[0];  // SAVE DATA
                    previousNickname = nicknames[0]; // SAVE DATA
                    previousCountry = countries[0]; // SAVE DATA

                    nicknames[0] = nickname; // REPLACE DATA
                    highScores[0] = scorePoints; // REPLACE DATA
                    countries[0] = country; // REPLACE DATA
                ...
                for (i = 1; i < highScores.length; i++)
                {
                   if (scorePoints > highScores[i] && scorePoints < highScores[i-1])
                    {   //replace the previous highscore
                        previousScore = highScores[i]; // SAVE DATA
                        previousNickname = nicknames[i]; // SAVE DATA
                        previousCountry = countries[i]; // SAVE DATA
                        
                        nicknames[i] = nickname; // REPLACE DATA
                        highScores[i] = scorePoints; // REPLACE DATA
                        countries[i] = country; // REPLACE DATA
                ...
                //store highscores        
                localStorage.setItem("highScores", JSON.stringify(highScores)); // Save the array in the local storage
                localStorage.setItem("nicknames", JSON.stringify(nicknames)); // Save the array in the local storage
                localStorage.setItem("countries", JSON.stringify(countries)); // Save the array in the local storage
            ...
            for (i = 0; i < highScores.length; i++)
            {
                ...
                ctx.fillText(nicknames[i] + " - " + parseInt(highScores[i]) + " pts " + countries[i], 12, canvas.height/4 + (i*30)); 
            }
    	...
    	if(powerofgods)
        {
            ctx.drawImage(PowerOfGods, 0, 0); // Draw the monster 2
